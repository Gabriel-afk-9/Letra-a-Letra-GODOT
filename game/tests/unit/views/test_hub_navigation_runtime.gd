...
func test_select_each_page_shows_identifying_text() -> void:
	for page_id in PAGES:
		...
		if page_id == &"inventory":
			assert_not_null(page.get_node("Content/MainVBox/TabScroll/TabsRow/COSMETICSTab"), "inventário deve ter seção cosméticos")
			assert_not_null(page.get_node("Content/MainVBox/Panel/PanelMargin/PanelVBox/SubTabsRow/AVATARTab"), "inventário deve ter subseletor de avatar")
